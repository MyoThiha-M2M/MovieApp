...
if (isset($_GET['deTheaterID'])) {
    ...
    $select = "SELECT * FROM Theaters WHERE TheaterID = $detailTheaterID";
...
<body>
    ...
    <div class="main-content">
        <section id="parallex" class="register-parallax" style="background: url(theaterImages/<?php echo $theaterImg ?>) center center;
                    ...
                    <div class="col-lg-10">
                        <div class="parallax-form">
                            <div class="DetailWrapper">
                                <img src="theaterImages/<?php echo $theaterImg ?>" alt="" class="img-fluid w-100" />
                                <div class="DetailContainer" id="theaterDetailContainer">
                                    <h2><?php echo $theaterName ?></h2>
                                    <div class="flexContainer">
                                        <div>Theater Name: <span><?php echo $theaterName ?></span></div>
                                        <div>Theater Type: <span><?php echo $theaterType ?></span></div>
                                    </div>
                                    <div class="flexContainer">

                                        <div>Contact number: <span><?php echo $contactNumber ?></span></div>
                                        <div>Location: <span><?php echo $location ?></span></div>
                                    </div>
                                </div>
                            </div>
                            <div class="OverviewContainer">
                                <h2><?php echo $theaterType ?></h2> <br>
                                <p><?php echo $description ?></p>
                            </div>
                            <div class="currentlyShowingMovieContainer">
                                <ul class="favorites-slider list-inline row p-0 mb-0">
                                    <?php
                                    $select = "SELECT distinct m.*, g.GenreName, f.FormatName FROM Shows s, Theaters t, Movies m, Genres g, Formats f WHERE s.TheaterID = $theaterID
                                     AND s.TheaterID = t.TheaterID AND s.MovieID = m.MovieID AND g.GenreID = m.GenreID AND f.FormatID = m.FormatID";
                                    $query = mysqli_query($connect, $select);
                                    $count = mysqli_num_rows($query);
                                    if ($count > 0) {
                                        for ($i = 0; $i < $count; $i++) {
                                            $row = mysqli_fetch_array($query);
                                            $showMovieID = $row['MovieID'];
                                            $showMovieName = $row['MovieName'];
                                            $showMovieGenreID = $row['GenreID'];
                                            $showMovieFormatID = $row['FormatID'];
                                            $showMovieGenreName = $row['GenreName'];
                                            $showMovieFormatName = $row['FormatName'];
                                            $duration = $row['Duration'];
                                            $hour = substr($duration, 1, 1) . 'hr';
                                            $minute = substr($duration, 3, 2) . 'min';
                                            $durationText = $hour . " " . $minute;
                                            $showMoviePoster = $row['Poster1'];
                                    ?>
                                    <li class="slide-item">
                                        <div class="block-images position-relative">
                                            <div class="img-box">
                                                <img src="moviePosters/<?php echo $showMoviePoster ?>" class="img-fluid"
                                                    alt="" />
                                            </div>
                                            <div class="block-description">
                                                <h6 class="iq-title">
                                                    <a href="#"> <?php echo $showMovieName ?> </a>
                                                </h6>
                                                <div class="movie-time d-flex align-items-center my-2">
                                                    <div class="badge badge-secondary p-1 mr-2">
                                                        <?php echo $showMovieGenreName ?>
                                                    </div>
                                                    <span class="text-white"><?php echo $durationText ?></span>
                                                </div>
                                                <div class="hover-buttons">
                                                    <a href="movieDetail.php?deMovieID=<?php echo $showMovieID ?>"
